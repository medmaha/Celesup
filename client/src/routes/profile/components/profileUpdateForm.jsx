...
export default function ProfileUpdateFormWrapper({
    ...
    async function handleFormSubmit(ev) {
        ...
        if (profile.cover_img !== formData.cover) {
            form.append("cover_img", formData.cover)
        } else {
            form.append("cover_img", "")
        }
...
function ProfileUpdateImagesFields({ formData }) {
    function changeProfileImage(ev) {
        if (
            ...
            const target =
                ev.target.closest(".img__btn") ||
                (ev.target.classList.contains("img__btn") && ev.target)
            if (!target) return

            const fileInput = document.createElement("input")
            fileInput.setAttribute("type", "file")

            function changeImage(file, action) {
                const image = target
                    .closest(".profile__images")
                    ?.querySelector(
                        `img.${target.getAttribute("data-select-image")}`,
                    )

                if (!!file.files.length && image) {
                    ...
                    reader.addEventListener(
                        ...
                        () => {
                            ...
                            formData[action] = file.files[0]

                            // setFormData({
                            //     ...formData,
                            //     [target.getAttribute("data-select-image")]:
                            //         file.files[0],
                            // })
                        },
                        false,
            ...
            switch (target.getAttribute("data-select-image")) {
                case "avatar":
                    ...
                    fileInput.addEventListener("change", ({ target }) =>
                        changeImage(target, "avatar"),
                    )
                    break
                case "cover":
                    fileInput.click()
                    fileInput.addEventListener("change", ({ target }) =>
                        changeImage(target, "cover"),
                    )
                    break
                case "remove":
                    // fileInput.click()
