...
const SignupExtraData = () => {
    const [gender, dispatch] = useReducer(toggleGender, {male:false, female:false})
    ...
    return (
        <div className="signup__extra__info mt-3 height-100 d-flex justify-content-center">
            <div className="card mx-__" style={{maxWidth: '350px'}}>
                ...
                {data && <DisplayAlertMessage asSuccess={true} message={'Successfully Registered your account'}/>}

                <div className="input-field"> 
                    <input  onChange={handleFormChange} type="text" name='full_name' placeholder='full name' autoComplete='off' />
                </div>
                <div className="input-field">
                    <textarea onChange={handleFormChange} name='biography' placeholder='biography (optional)' autoComplete='off'></textarea>
                </div>
                
                <div className="input-field">
                    <input onChange={handleFormChange} type="text" name='city' placeholder='city (optional)' autoComplete='off'/>
                </div>
                <div className="px-__">
                    <span className='' style={{letterSpacing: '1px'}}>Gender?</span>
                    <div className="pl-1 py-__">
                        <label>
                            <input name="male" type="checkbox" checked={gender.male} onChange={handleGenderChange} />
                            <span className='px-__'>Male</span>
                        </label>
                        <label className='ml-1'>
                            <input name="female" type="checkbox" checked={gender.female} onChange={handleGenderChange} />
                            <span className='px-__'>Female</span>
                        </label>
                    </div>
                </div>
                <div className={formData.full_name?.length > 3 ? "pt-1 d-flex align-items-center justify-content-between" :"pt-1 d-flex align-items-center justify-content-center"}>
                    <div onClick={handleAvatarChange} className="default__avater cursor-pointer  pos-relative">
                        <img ref={avatar} src={require('./default.png')} className='responsive br-full border' alt="File Not Supported" / >
                        <span onClick={handleAvatarChange} className='cursor-pointer pos-absolute px-__ bottom-10 right-minus-20'>
                            <span onClick={handleAvatarChange} className='icon-wrapper br-full'>
