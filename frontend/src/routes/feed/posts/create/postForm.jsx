...
const PostForm = () => {
	...
	function uploadFile() {
		...
		hiddenFileInput.addEventListener('change', ({ target }) => {
			if (target.files && target.files[0]) {
				const ev = { target: { name: 'picture', value: target.files[0] } }
				handleFormChange(ev)
	...
	function handleFormChange({ target }) {
		updateFormData({
			...
			valid: formData.caption.length > 2 || formData.excerpt.length > 2 || formData.picture,
		})
	}
			...
			{fileSelected ? (
				...
				<span className='d-block height-100'>
					<div className='header  px-1 pos-relative pb-__'>
						...
						<div onClick={() => context.setFocusState(null)} className='br-full p-__ pos-absolute black right-10-px top-10-px'>
							<span className='icon-wrapper white-text cursor-pointer'>
					...
					<span className='divider' style={{ backgroundColor: 'grey' }}></span>
					...
					<div className='post__form__body'>
						<div className='author pt-1 d-flex align-items-center px-1'>
							...
							<div className='pl-__ d-flex flex-column'>
								<div className='pb-__' style={{ marginBottom: '2px' }}>
									{context.user?.full_name ? <span>{context.user.full_name}</span> : <span>@{context.user.username}</span>}
								</div>
						...
						<div ref={postData} className='px-1 d-flex flex-column gap-1-rem'>
							<span>
								<input
									type='text'
									name='caption'
							...
							<span>
								<TextArea name='excerpt' placeholder='Add a Description' row={6} onChange={handleFormChange} />
							</span>
							<span>
								<button className='btn blue br-lg' onClick={() => setAddHashTag((prev) => !prev)}>
									{!addHashtag ? 'Add hashtags' : <>{formData.hashtags?.split(',').length > 1 ? 'Hashtags' : 'Hashtag'}</>}
								</button>
							</span>

							<span>{addHashtag && <Hashtag name='hashtags' placeholder='Add hashtags! separated by comma' onChange={handleFormChange} />}</span>
						</div>
					</div>
					...
					<div className='pos-absolute d-flex justify-content-center  flex-column bottom-8-px left-0 height-4-rem width-100'>
						...
						<div className='d-flex justify-content-between align-items-center height-100 mx-1'>
							<div className='d-flex justify-content-center align-items-start'>
								<span className='icon-wrapper cursor-pointer' title='Image' onClick={uploadFile}>
									<svg className='_blue-icon' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512'>
										<path d='M447.1 32h-384C28.64 32-.0091 60.65-.0091 96v320c0 35.35 28.65 64 63.1 64h384c35.35 0 64-28.65 64-64V96C511.1 60.65 483.3 32 447.1 32zM111.1 96c26.51 0 48 21.49 48 48S138.5 192 111.1 192s-48-21.49-48-48S85.48 96 111.1 96zM446.1 407.6C443.3 412.8 437.9 416 432 416H82.01c-6.021 0-11.53-3.379-14.26-8.75c-2.73-5.367-2.215-11.81 1.334-16.68l70-96C142.1 290.4 146.9 288 152 288s9.916 2.441 12.93 6.574l32.46 44.51l93.3-139.1C293.7 194.7 298.7 192 304 192s10.35 2.672 13.31 7.125l128 192C448.6 396 448.9 402.3 446.1 407.6z' />
									</svg>
								</span>
								<span className='icon-wrapper cursor-pointer ml-__' title='Video'>
									<svg className='_green-icon' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 448 512'>
										<path d='M186.8 202.1l95.2 54.1-95.2 54.1V202.1zM448 80v352c0 26.5-21.5 48-48 48H48c-26.5 0-48-21.5-48-48V80c0-26.5 21.5-48 48-48h352c26.5 0 48 21.5 48 48zm-42 176.3s0-59.6-7.6-88.2c-4.2-15.8-16.5-28.2-32.2-32.4C337.9 128 224 128 224 128s-113.9 0-142.2 7.7c-15.7 4.2-28 16.6-32.2 32.4-7.6 28.5-7.6 88.2-7.6 88.2s0 59.6 7.6 88.2c4.2 15.8 16.5 27.7 32.2 31.9C110.1 384 224 384 224 384s113.9 0 142.2-7.7c15.7-4.2 28-16.1 32.2-31.9 7.6-28.5 7.6-88.1 7.6-88.1z' />
									</svg>
								</span>
								<span className='icon-wrapper cursor-pointer ml-__' title='Events'>
									<svg className='_orange-icon' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 448 512'>
								...
								<span className='icon-wrapper cursor-pointer ml-__' title='Article'>
									<svg className='_red-icon' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512'>
							...
							<button className={`theme-primary px-2 py-__ br-lg opacity-2 ${!formData.valid && 'disabled'}`} onClick={submitForm}>
								Publish
							</button>
						</div>
					</div>
